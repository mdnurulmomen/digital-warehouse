...
<template v-if="userHasPermissionTo('view-product-asset-index')">
	<div class="pcoded-content">
							...
							<div class="form-row">
								<div class="form-group col-md-12">
									...
									<input type="text" 
										...
										:class="!errors.location.name  ? 'is-valid' : 'is-invalid'" 
		...
		-->	
 		...
 		<!-- View Modal -->
		<!-- 
		<div class="modal fade" id="product-location-view-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
	...
	export default {
		...
		methods : {
			...
			fetchAllContents() {
				...
				axios
					.get('/api/locations/' + this.perPage + "?page=" + this.pagination.current_page)
					.then(response => {
			...
			searchData(emitedValue=false) {
				...
				axios
				.get(
					"/api/search-locations/" + this.query + "/" + this.perPage + "?page=" + this.pagination.current_page
				)
				.then(response => {
					this.allFetchedContents = response.data;
			...
			showContentDetails(object) {	
				...
				$('#product-location-view-modal').modal('show');
			},
			*/
			...
			storeAsset(singleAssetData) {
				...
				axios
					.post('/locations/' + this.perPage, this.singleAssetData)
					.then(response => {
						if (response.status == 200) {
			...
			updateAsset(singleAssetData) {
				...
				axios
					.put('/locations/' + this.singleAssetData.id + '/' + this.perPage, this.singleAssetData)
					.then(response => {
						if (response.status == 200) {
			...
			deleteAsset(singleAssetData) {
				...
				axios
					.delete('/locations/' + singleAssetData.id + '/' + this.perPage, singleAssetData)
					.then(response => {
						if (response.status == 200) {
			...
			restoreAsset(singleAssetData) {
				...
				axios
					.patch('/locations/' + singleAssetData.id + '/' + this.perPage, singleAssetData)
					.then(response => {
						if (response.status == 200) {
			...
			importExcelFile(fileToExport) {
				...
				axios
					.post('/import-locations', fileToExport)
					.then(response => {
						if (response.status == 200) {
							this.$toastr.s("New locations has been stored", "Success");

							this.pagination.current_page = 1; 
							this.allFetchedContents = response.data;
							this.query !== '' ? this.searchData() : this.showSelectedTabContents();
							
							$('#locations-importing-modal').modal('hide');
						}
					})
					.catch(error => {
						if (error.response.status == 422) {
			...
			goCategoryProducts(productCategory) {
				...
				this.$router.push({ name: 'location-products', params: { location: productCategory, categoryName: productCategory.name.replace(/ /g,"-") }});
			},
			*/
			...
			verifyUserInput() {
				...
				if (this.errors.location.constructor === Object && Object.keys(this.errors.location).length == 0) {
					return true;
				}
				...
				switch(formInputName) {
					...
					case 'name' :
						...
						if (!this.singleAssetData.name.match(/^[_A-z0-9]*((-|&|\s)*[_A-z0-9])*$/g)) {
							this.errors.location.name = 'No special character';
						}
						else{
							this.submitForm = true;
							this.$delete(this.errors.location, 'name');
						}
